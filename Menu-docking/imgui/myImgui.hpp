...
void AddSliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0)
{
	...
	window->DrawList->AddCircleFilled(value_pos,4.5f, ImColor(ImVec4(config::menu_color::theme_col[0], config::menu_color::theme_col[1], config::menu_color::theme_col[2], 255))); // 滑块圆点
...
void AddHotkey(const char* label, int* v)
{
	...
	ImVec2 strSize = ImGui::CalcTextSize(keyStr.data(), NULL, true);			// 文本大小
	...
	{
		strSize = ImGui::CalcTextSize("...", NULL, true);			// 文本大小
		ImGui::SetCursorPos(ImVec2(cursorpos.x + (ContentX - strSize.x - g.Style.FramePadding.x * 3.f), cursorpos.y + 3.f));
